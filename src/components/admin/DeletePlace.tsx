...
import { removePlace } from "@/actions/placeAction";

export default function DeletePlace({ placeID }: { placeID: string }) {
  const appContext = React.useContext(context);

  async function handelDelete() {
    const response = await removePlace(placeID);
    alert(response);
    if (response === "Place deleted successfully") {
