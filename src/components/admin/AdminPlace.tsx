...
export default function AdminPlace({
  ...
  return (
    <div className="relative h-60 cursor-pointer overflow-hidden rounded-lg shadow-md">
      <Link href={makeURL(place.name, place.id)} className="group">
        <img
      ...
      <div className="absolute bottom-0 flex w-full items-center justify-between p-3">
        <button
          ...
          onClick={() => appContext?.setPopUp(<EditPlace />)}
        >
          <svg
        ...
        <button
          className="cursor-pointer rounded-full bg-gray-900 p-2"
          onClick={() =>
            appContext?.setPopUp(<DeletePlace placeID={place.id} />)
          }
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
